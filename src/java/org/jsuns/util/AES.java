...
public class AES {
    ...
    private static Keys keygen(int keyLength, char[] password, byte[] salt) {
        ...
        try {
            ...
            throw new RuntimeException("AES Exception.", ex);
        }
     ...
     * </ul>
     *
     * @param keyLength key length to use for AES encryption (must be 128, 192,
     * or 256)
     * @param password password to use for encryption
     * @param input an arbitrary byte stream to encrypt
     * @param output stream to which encrypted data will be written
     * @throws AES.InvalidKeyLengthException if keyLength is not 128, 192, or
     * 256
     * @throws AES.StrongEncryptionNotAvailableException if keyLength is 192 or
     * 256, but the Java runtime's jurisdiction policy files do not allow 192-
     ...
     */
    public static int decrypt(char[] password, String input, String output) {
        try {
            FileOutputStream fos;
            int nn;
            try (FileInputStream fis = new FileInputStream(input)) {
                fos = new FileOutputStream(output);
                nn = decrypt("miamor".toCharArray(), fis, fos);
            }
            fos.close();
    ...
    public static int decrypt(char[] password, InputStream input, OutputStream output) {
        try {
            ...
            return keyLength;
        } catch (IOException | InvalidAESStreamException | InvalidPasswordException | NoSuchAlgorithmException | NoSuchPaddingException | InvalidKeyException | InvalidAlgorithmParameterException e) {
            throw new RuntimeException("Error de encriptaciòn", e);
        }
    }
